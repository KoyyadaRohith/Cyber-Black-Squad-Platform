...
const projectsModule = {
  ...
  handleSaveSubmit(e) {
    ...
    window.cbsDB.saveProject(projectData);
  ...
  handleArchive(id) {
    ...
    if (confirm('Archive this project?')) {
      ...
      if (p) {
        ...
        window.cbsDB.addNotification('Projects', 'Project Archived', `Project "${p.name}" has been archived.`, 'Medium');
        window.cbsDB.addActivityLog(window.cbsAuth.getCurrentUser()?.id, `Archived project: ${p.name}`, 'Project');
        window.cbsApp?.sync();
  ...
  handleRestore(id) {
    ...
    if (p) {
      p.status = 'Planning';
      window.cbsDB.saveProject(p);
      window.cbsDB.addNotification('Projects', 'Project Restored', `Project "${p.name}" restored back to planning status.`, 'Medium');
      window.cbsDB.addActivityLog(window.cbsAuth.getCurrentUser()?.id, `Restored archived project: ${p.name}`, 'Project');
      window.cbsApp?.sync();
  ...
  handleDelete(id) {
    ...
    if (confirm('Permanently delete project record?')) {
      const p = window.cbsDB.getProjectById(id);
      if (p) {
        window.cbsDB.deleteProject(id);
        
        const milestones = window.cbsDB.getMilestones().filter(m => m.projectId !== id);
        localStorage.setItem('cbs_milestones_coll', JSON.stringify(milestones));
  ...
  handleSaveMilestoneInlineSubmit(e) {
    e.preventDefault();
    if (!window.cbsAuth.canEdit()) return;
    ...
    window.cbsDB.saveMilestone(msData);
    window.cbsApp?.sync();
  ...
  handleDeleteMilestone(id) {
    ...
    if (confirm('Delete this milestone?')) {
      window.cbsDB.deleteMilestone(id);
      window.cbsApp?.sync();
      this.switchDetailTab('milestones');
    }
  },

  handleUploadDocFile(input) {
    if (!window.cbsAuth.canEdit()) return;
    ...
    if (p) {
      ...
      window.cbsDB.addNotification('Projects', 'Document Uploaded', `File "${name}" uploaded to project "${p.name}".`, 'Low');
      window.cbsDB.addActivityLog(user.id, `Uploaded document "${name}" to project "${p.name}".`, 'Project');
