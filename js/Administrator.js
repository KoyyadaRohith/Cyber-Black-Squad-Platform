/**
 * Cyber Black Squad – Central Administrator Control Center Module
 * 
...
const AdministratorModule = {
  activeAdministratorSubTab: 'dashboard',
  charts: {},
  editingUserId: null,

  render() {
    // Role access authorization check (Administrators and Founders have access)
    const isAdministrator = window.cbsAuth.isAdministrator() || window.cbsAuth.isFounder();
    const sidebarItem = document.getElementById('sidebarAdministratorItem');
    ...
    const container = document.getElementById('view-Administrator');
    if (!container) return;
    ...
    container.innerHTML = `
      <div class="Administrator-wrapper">
        ...
        <aside class="Administrator-sub-sidebar">
          <div class="Administrator-sidebar-title"><i class="fa-solid fa-gears" style="margin-right:6px;"></i>System Controls</div>
          <button onclick="AdministratorModule.switchAdministratorTab('dashboard')" class="Administrator-nav-btn" id="adm-tab-dashboard"><i class="fa-solid fa-chart-line"></i> Dashboard</button>
          <button onclick="AdministratorModule.switchAdministratorTab('users')" class="Administrator-nav-btn" id="adm-tab-users"><i class="fa-solid fa-users"></i> Users Control</button>
          <button onclick="AdministratorModule.switchAdministratorTab('projects')" class="Administrator-nav-btn" id="adm-tab-projects"><i class="fa-solid fa-briefcase"></i> Projects Oversight</button>
          <button onclick="AdministratorModule.switchAdministratorTab('tasks')" class="Administrator-nav-btn" id="adm-tab-tasks"><i class="fa-solid fa-list-check"></i> Tasks Board</button>
          <button onclick="AdministratorModule.switchAdministratorTab('directory')" class="Administrator-nav-btn" id="adm-tab-directory"><i class="fa-solid fa-address-book"></i> Team Directory</button>
          <button onclick="AdministratorModule.switchAdministratorTab('channels')" class="Administrator-nav-btn" id="adm-tab-channels"><i class="fa-solid fa-comments"></i> Channels Roster</button>
          <button onclick="AdministratorModule.switchAdministratorTab('notifications')" class="Administrator-nav-btn" id="adm-tab-notifications"><i class="fa-regular fa-bell"></i> Alerts Center</button>
          <button onclick="AdministratorModule.switchAdministratorTab('activity')" class="Administrator-nav-btn" id="adm-tab-activity"><i class="fa-solid fa-clock-rotate-left"></i> System Logs</button>
          <button onclick="AdministratorModule.switchAdministratorTab('analytics')" class="Administrator-nav-btn" id="adm-tab-analytics"><i class="fa-solid fa-chart-pie"></i> Analytics & CSV</button>
          <button onclick="AdministratorModule.switchAdministratorTab('broadcasts')" class="Administrator-nav-btn" id="adm-tab-broadcasts"><i class="fa-solid fa-bullhorn"></i> Broadcast Center</button>
          <button onclick="AdministratorModule.switchAdministratorTab('security')" class="Administrator-nav-btn" id="adm-tab-security"><i class="fa-solid fa-shield-halved"></i> Session Security</button>
          <button onclick="AdministratorModule.switchAdministratorTab('settings')" class="Administrator-nav-btn" id="adm-tab-settings"><i class="fa-solid fa-sliders"></i> Global Settings</button>
        </aside>

        <!-- Dynamic Content Panel -->
        <div class="Administrator-content-pane" id="AdministratorTabContentArea">
          <!-- Loaded dynamically -->
        </div>
      </div>
    `;

    this.switchAdministratorTab(this.activeAdministratorSubTab);
  },

  injectStyles() {
    if (document.getElementById('cbs-Administrator-styles-override')) return;
    const style = document.createElement('style');
    style.id = 'cbs-Administrator-styles-override';
    style.textContent = `
      ...
      .Administrator-nav-btn i {
        width: 16px;
        text-align: center;
        font-size: 14px;
      }
      .Administrator-nav-btn:hover {
        background: var(--bg-main);
        color: var(--primary-color);
      }
      .Administrator-nav-btn.active {
        background: rgba(59, 130, 246, 0.08);
        color: var(--primary-color);
        font-weight: 600;
      }
      body.dark-theme .Administrator-nav-btn.active {
        background: rgba(59, 130, 246, 0.15);
      }
      .Administrator-grid-charts {
        display: grid;
        grid-template-columns: 1fr 1fr;
        gap: 20px;
      }
      .Administrator-chart-box {
        background: var(--bg-main);
        border: 1px solid var(--border-color);
        border-radius: var(--radius-md);
        padding: 16px;
  ...
  switchAdministratorTab(tabName) {
    this.activeAdministratorSubTab = tabName;

    // Toggle nav active headings
    document.querySelectorAll('.Administrator-nav-btn').forEach(btn => {
    ...
    const area = document.getElementById('AdministratorTabContentArea');
    if (!area) return;
    ...
    area.innerHTML = `
      <div style="display:flex; flex-direction:column; gap:20px;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          ...
          <span class="badge badge-purple" style="font-size:11px;">Administrator oversight view</span>
        </div>
        ...
        <div style="border-top:1px dashed var(--border-color); padding-top:20px;">
          ...
          <div style="display:flex; gap:12px; flex-wrap:wrap;">
            <button onclick="AdministratorModule.switchAdministratorTab('users'); AdministratorModule.openCreateUserFormInline();" class="btn-primary" style="padding:8px 16px; font-size:12px;"><i class="fa-solid fa-user-plus"></i> Create User</button>
            ...
            <button onclick="AdministratorModule.switchAdministratorTab('broadcasts')" class="btn-primary" style="padding:8px 16px; font-size:12px; background:var(--color-danger);"><i class="fa-solid fa-bullhorn"></i> Broadcast Announcement</button>
          </div>
        </div>
      </div>
    `;
  ...
  renderUsers(area) {
    ...
    filtered.forEach(u => {
      ...
      if (!isSelf) {
        if (isActive) {
          statusBtn = `<button onclick="AdministratorModule.handleToggleUserStatus('${u.id}', 'suspended')" class="badge badge-orange" style="border:none; cursor:pointer;">Suspend</button>`;
        } else {
          statusBtn = `<button onclick="AdministratorModule.handleToggleUserStatus('${u.id}', 'active')" class="badge badge-green" style="border:none; cursor:pointer;">Activate</button>`;
        }
      } else {
        statusBtn = `<span class="badge badge-purple" style="font-size:9.5px;">Self Account</span>`;
      }

      const deleteBtn = (!isSelf) ? `<button onclick="AdministratorModule.handleDeleteUser('${u.id}')" class="action-icon-btn delete" title="Delete User"><i class="fa-regular fa-trash-can"></i></button>` : '';
        ...
        <tr>
          ...
          <td>
            <div class="card-actions" style="gap:8px;">
              <button onclick="AdministratorModule.openEditUserForm('${u.id}')" class="action-icon-btn" title="Edit Profile Details"><i class="fa-regular fa-pen-to-square"></i></button>
              <button onclick="AdministratorModule.handleResetPassword('${u.id}')" class="action-icon-btn" title="Reset Password credentials"><i class="fa-solid fa-key" style="color:var(--color-warning);"></i></button>
              ${statusBtn}
              ${deleteBtn}
            </div>
          </td>
    ...
    area.innerHTML = `
      <div style="display:flex; flex-direction:column; gap:16px;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          ...
          <button onclick="AdministratorModule.openCreateUserFormInline()" class="btn-primary" style="padding:6px 12px; font-size:11.5px;"><i class="fa-solid fa-user-plus"></i> Add User</button>
        </div>
        ...
        <div style="display:flex; gap:10px; flex-wrap:wrap; align-items:center; background:var(--bg-main); padding:12px; border-radius:var(--radius-md); border:1px solid var(--border-color); margin-bottom:20px;">
          <div class="topbar-search" style="display:block; width:180px;">
            ...
            <input type="text" id="admUsersSearch" placeholder="Search accounts..." value="${searchVal}" oninput="AdministratorModule.switchAdministratorTab('users')" style="padding: 6px 12px 6px 30px; font-size:11.5px;">
          </div>
          
          <select id="admUsersRoleFilter" onchange="AdministratorModule.switchAdministratorTab('users')" style="padding:6px; font-size:11.5px; border:1px solid var(--border-color); border-radius:var(--radius-sm);">
            <option value="Workspace Roles" ${roleVal === 'Workspace Roles' ? 'selected' : ''}>All Role</option>
            <option value="Founder & CEO" ${roleVal === 'Founder & CEO' ? 'selected' : ''}>Founder &amp; CEO</option>
            <option value="Co-Founder & CTO" ${roleVal === 'Co-Founder & CTO' ? 'selected' : ''}>Co-Founder &amp; CTO</option>
            <option value="Administrator" ${roleVal === 'Administrator' ? 'selected' : ''}>Administrator</option>
            <option value="Management Lead" ${roleVal === 'Management Lead' ? 'selected' : ''}>Management Lead</option>
            <option value="Marketing & Growth Lead" ${roleVal === 'Marketing & Growth Lead' ? 'selected' : ''}>Marketing &amp; Growth Lead</option>
            <option value="Finance & Compliance Lead" ${roleVal === 'Finance & Compliance Lead' ? 'selected' : ''}>Finance &amp; Compliance Lead</option>
          ...
          <select id="admUsersDeptFilter" onchange="AdministratorModule.switchAdministratorTab('users')" style="padding:6px; font-size:11.5px; border:1px solid var(--border-color); border-radius:var(--radius-sm);">
            <option value="All" ${deptVal === 'All' ? 'selected' : ''}>All Departments</option>
            <option value="Executive Leadership" ${deptVal === 'Executive Leadership' ? 'selected' : ''}>Executive Leadership</option>
            <option value="Administration & Operations" ${deptVal === 'Administration & Operations' ? 'selected' : ''}>Administration &amp; Operations</option>
            <option value="Management & Business Operations" ${deptVal === 'Management & Business Operations' ? 'selected' : ''}>Management &amp; Business Operations</option>
            <option value="Marketing & Communications" ${deptVal === 'Marketing & Communications' ? 'selected' : ''}>Marketing &amp; Communications</option>
            <option value="Finance & Compliance" ${deptVal === 'Finance & Compliance' ? 'selected' : ''}>Finance &amp; Compliance</option>
            <option value="Cybersecurity Operations" ${deptVal === 'Cybersecurity Operations' ? 'selected' : ''}>Cybersecurity Operations</option>
            <option value="Artificial Intelligence & Machine Learning" ${deptVal === 'Artificial Intelligence & Machine Learning' ? 'selected' : ''}>Artificial Intelligence &amp; Machine Learning</option>
            <option value="Software Engineering" ${deptVal === 'Software Engineering' ? 'selected' : ''}>Software Engineering</option>
            <option value="Project Management" ${deptVal === 'Project Management' ? 'selected' : ''}>Project Management</option>
            <option value="Research & Development" ${deptVal === 'Research & Development' ? 'selected' : ''}>Research &amp; Development</option>
          </select>

          <select id="admUsersStatusFilter" onchange="AdministratorModule.switchAdministratorTab('users')" style="padding:6px; font-size:11.5px; border:1px solid var(--border-color); border-radius:var(--radius-sm);">
          ...
          <select id="admUsersSort" onchange="AdministratorModule.switchAdministratorTab('users')" style="padding:6px; font-size:11.5px; border:1px solid var(--border-color); border-radius:var(--radius-sm);">
  ...
  openCreateUserFormInline() {
    ...
    container.innerHTML = `
      <div class="Administrator-inline-form-box">
        ...
        <form onsubmit="AdministratorModule.handleSaveUserInlineSubmit(event)" style="display:flex; flex-direction:column; gap:12px;">
          ...
          <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:12px;">
            <div class="form-group" style="margin-bottom:0;">
              ...
              <select id="inlineUsRole" style="padding:6px; font-size:12px;">
                ...
                <option value="Marketing & Growth Lead">Marketing &amp; Growth Lead</option>
                <option value="Finance & Compliance Lead">Finance &amp; Compliance Lead</option>
  ...
  openEditUserForm(userId) {
    ...
    container.innerHTML = `
      <div class="Administrator-inline-form-box" style="border-color:var(--primary-color);">
        ...
        <form onsubmit="AdministratorModule.handleSaveUserInlineSubmit(event)" style="display:flex; flex-direction:column; gap:12px;">
          ...
          <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:12px;">
            <div class="form-group" style="margin-bottom:0;">
              ...
              <select id="inlineUsRole" style="padding:6px; font-size:12px;">
                <option value="Founder & CEO" ${u.role === 'Founder & CEO' ? 'selected' : ''}>Founder &amp; CEO</option>
                <option value="Co-Founder & CTO" ${u.role === 'Co-Founder & CTO' ? 'selected' : ''}>Co-Founder &amp; CTO</option>
                <option value="Administrator" ${u.role === 'Administrator' ? 'selected' : ''}>Administrator</option>
                <option value="Management Lead" ${u.role === 'Management Lead' ? 'selected' : ''}>Management Lead</option>
                <option value="Marketing & Growth Lead" ${u.role === 'Marketing & Growth Lead' ? 'selected' : ''}>Marketing &amp; Growth Lead</option>
                <option value="Finance & Compliance Lead" ${u.role === 'Finance & Compliance Lead' ? 'selected' : ''}>Finance &amp; Compliance Lead</option>
            ...
            <div class="form-group" style="margin-bottom:0;">
              <label style="font-size:11px;">Workspace Department</label>
              <select id="inlineUsDept" style="padding:6px; font-size:12px;">
                <option value="Executive Leadership" ${u.department === 'Executive Leadership' ? 'selected' : ''}>Executive Leadership</option>
                <option value="Administration & Operations" ${u.department === 'Administration & Operations' ? 'selected' : ''}>Administration &amp; Operations</option>
                <option value="Management & Business Operations" ${u.department === 'Management & Business Operations' ? 'selected' : ''}>Management &amp; Business Operations</option>
                <option value="Marketing & Communications" ${u.department === 'Marketing & Communications' ? 'selected' : ''}>Marketing &amp; Communications</option>
                <option value="Finance & Compliance" ${u.department === 'Finance & Compliance' ? 'selected' : ''}>Finance &amp; Compliance</option>
                <option value="Cybersecurity Operations" ${u.department === 'Cybersecurity Operations' ? 'selected' : ''}>Cybersecurity Operations</option>
                <option value="Artificial Intelligence & Machine Learning" ${u.department === 'Artificial Intelligence & Machine Learning' ? 'selected' : ''}>Artificial Intelligence &amp; Machine Learning</option>
                <option value="Software Engineering" ${u.department === 'Software Engineering' ? 'selected' : ''}>Software Engineering</option>
                <option value="Project Management" ${u.department === 'Project Management' ? 'selected' : ''}>Project Management</option>
  ...
  async handleSaveUserInlineSubmit(e) {
    e.preventDefault();
    ...
    if (!this.editingUserId) {
      // Create path — preserve admin session during enrollment
      const pwd = document.getElementById('inlineUsPass').value;
      const adminSession = sessionStorage.getItem('cbs_session'); // snapshot admin session
      try {
        const u = await window.cbsAuth.register(name, email, pwd, role);
        u.department = dept;
        await window.cbsDB.saveUser(u);
        // Restore admin session if it was disrupted
        if (!window.cbsAuth.isAuthenticated() && adminSession) {
          sessionStorage.setItem('cbs_session', adminSession);
        }
        window.cbsApp?.sync();
  ...
  async handleToggleUserStatus(userId, status) {
    await window.cbsDB.setUserStatus(userId, status);

    // If user suspended, force session logout
    if (status === 'suspended') {
      const sessions = window.cbsDB.getSessions();
      await Promise.all(sessions.filter(s => s.userId === userId).map(s => window.cbsDB.updateSessionStatus(s.id, 'Terminated')));
    }
  ...
  async handleDeleteUser(id) {
    if (confirm('Verify: Permanently delete this account record? This is irreversible.')) {
      await window.cbsDB.deleteUser(id);
      window.cbsApp?.sync();
    }
  },
  ...
  async handleResetPassword(id) {
    ...
    if (newPwd) {
      ...
      if (u) {
        u.password = newPwd;
        await window.cbsDB.saveUser(u);
        await window.cbsDB.addActivityLog(window.cbsAuth.getCurrentUser()?.id, `Administratively reset password for ${u.name}`, 'Security');
  ...
  renderProjects(area) {
    ...
    projects.forEach(p => {
      ...
      let managersSelect = `<select onchange="AdministratorModule.handleSaveProjectInline('${p.id}', 'ownerId', this.value)" style="padding:4px; font-size:12px; font-weight:600; border-radius:var(--radius-sm); border:1px solid var(--border-color); background:var(--bg-card); width:130px;">`;
      team.forEach(u => {
      ...
      let prioritySelect = `<select onchange="AdministratorModule.handleSaveProjectInline('${p.id}', 'priority', this.value)" style="padding:4px; font-size:12px; font-weight:600; border-radius:var(--radius-sm); border:1px solid var(--border-color); background:var(--bg-card); width:95px;">`;
      ...
      // Status selector
      let statusSelect = `<select onchange="AdministratorModule.handleSaveProjectInline('${p.id}', 'status', this.value)" style="padding:4px; font-size:12px; font-weight:600; border-radius:var(--radius-sm); border:1px solid var(--border-color); background:var(--bg-card); width:110px;">`;
  ...
  async handleSaveProjectInline(projId, field, value) {
  ...
  renderTasks(area) {
    ...
    tasks.forEach(t => {
      ...
      // Assignee dropdown
      let assigneeSelect = `<select onchange="AdministratorModule.handleSaveTaskInline('${t.id}', 'assigneeId', this.value)" style="padding:4px; font-size:12px; border-radius:var(--radius-sm); border:1px solid var(--border-color); background:var(--bg-card); width:130px;">`;
      team.forEach(u => {
      ...
      // Priorities dropdown
      let prioritySelect = `<select onchange="AdministratorModule.handleSaveTaskInline('${t.id}', 'priority', this.value)" style="padding:4px; font-size:12px; border-radius:var(--radius-sm); border:1px solid var(--border-color); background:var(--bg-card); width:95px;">`;
      ...
      // Status dropdown
      let statusSelect = `<select onchange="AdministratorModule.handleSaveTaskInline('${t.id}', 'status', this.value)" style="padding:4px; font-size:12px; border-radius:var(--radius-sm); border:1px solid var(--border-color); background:var(--bg-card); width:110px;">`;
      ...
      rowsHtml += `
        <tr style="opacity: ${isCompleted ? '0.7' : '1'};">
          <td>
            <div style="display:flex; align-items:center; gap:8px;">
              <input type="checkbox" ${isCompleted ? 'checked' : ''} onclick="AdministratorModule.handleToggleTaskComplete('${t.id}')" style="cursor:pointer;">
          ...
          <td>
            <div class="card-actions" style="gap:8px;">
              ...
              <button onclick="AdministratorModule.handleDeleteTask('${t.id}')" class="action-icon-btn delete" title="Wipe Task"><i class="fa-regular fa-trash-can"></i></button>
            </div>
          </td>
        </tr>
      `;
  ...
  async handleSaveTaskInline(id, field, value) {
  ...
  async handleToggleTaskComplete(id) {
    const t = window.cbsDB.getTaskById(id);
    if (t) {
  ...
  async handleDeleteTask(id) {
    if (confirm('Delete task permanently from database?')) {
      await window.cbsDB.deleteTask(id);
      window.cbsApp?.sync();
    }
  },
  ...
  renderDirectory(area) {
    ...
    filtered.forEach(u => {
      ...
      let assignSelect = `<select onchange="AdministratorModule.handleAssignUserProject('${u.id}', this.value)" style="padding:4px; font-size:11px; border-radius:var(--radius-sm); border:1px solid var(--border-color); background:var(--bg-card); width:130px; margin-top:6px;">`;
        ...
        <div style="background:var(--bg-main); border:1px solid var(--border-color); border-radius:var(--radius-lg); padding:16px; display:flex; flex-direction:column; gap:10px; position:relative; box-shadow:var(--shadow-sm);">
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; border-top:1px dashed var(--border-color); padding-top:10px; margin-top:6px;">
            <div style="display:flex; gap:8px;">
              <button onclick="AdministratorModule.openViewProfileModal('${u.id}')" class="badge badge-purple" style="border:none; cursor:pointer;">Profile</button>
              <button onclick="AdministratorModule.openEditProfileModal('${u.id}')" class="badge badge-blue" style="border:none; cursor:pointer;">Edit</button>
    ...
    area.innerHTML = `
      <div style="display:flex; flex-direction:column; gap:16px;">
        ...
        <!-- Filters Row -->
        <div style="display:flex; gap:10px; flex-wrap:wrap; align-items:center; background:var(--bg-main); padding:12px; border-radius:var(--radius-md); border:1px solid var(--border-color); margin-bottom:20px;">
          <select id="admDirRoleFilter" onchange="AdministratorModule.switchAdministratorTab('directory')" style="padding:6px; font-size:11.5px; border:1px solid var(--border-color); border-radius:var(--radius-sm);">
          <option value="All" ${roleVal === 'All' ? 'selected' : ''}>All Roles</option>
          <option value="Founder & CEO" ${roleVal === 'Founder & CEO' ? 'selected' : ''}>Founder &amp; CEO</option>
          <option value="Co-Founder & CTO" ${roleVal === 'Co-Founder & CTO' ? 'selected' : ''}>Co-Founder &amp; CTO</option>
          <option value="Administrator" ${roleVal === 'Administrator' ? 'selected' : ''}>Administrator</option>
          <option value="Management Lead" ${roleVal === 'Management Lead' ? 'selected' : ''}>Management Lead</option>
          ...
          <select id="admDirDeptFilter" onchange="AdministratorModule.switchAdministratorTab('directory')" style="padding:6px; font-size:11.5px; border:1px solid var(--border-color); border-radius:var(--radius-sm);">
            <option value="All" ${deptVal === 'All' ? 'selected' : ''}>All Departments</option>
            <option value="Executive Leadership" ${deptVal === 'Executive Leadership' ? 'selected' : ''}>Executive Leadership</option>
            <option value="Administration & Operations" ${deptVal === 'Administration & Operations' ? 'selected' : ''}>Administration &amp; Operations</option>
            <option value="Management & Business Operations" ${deptVal === 'Management & Business Operations' ? 'selected' : ''}>Management &amp; Business Operations</option>
          ...
          <select id="admDirStatusFilter" onchange="AdministratorModule.switchAdministratorTab('directory')" style="padding:6px; font-size:11.5px; border:1px solid var(--border-color); border-radius:var(--radius-sm);">
            <option value="All" ${statusVal === 'All' ? 'selected' : ''}>All Statuses</option>
            <option value="active" ${statusVal === 'active' ? 'selected' : ''}>Active</option>
            <option value="suspended" ${statusVal === 'suspended' ? 'selected' : ''}>Suspended</option>
          </select>
  ...
  async handleAssignUserProject(userId, projId) {
    if (!projId) return;
    const p = window.cbsDB.getProjectById(projId);
    if (p) {
      p.members = p.members || [];
      if (!p.members.includes(userId)) {
        ...
        await window.cbsDB.saveProject(p);
        await window.cbsDB.addNotification('Projects', 'Member Assigned', `${window.cbsDB.getUserById(userId)?.name} assigned to project ${p.name}.`, 'Medium');
        window.cbsApp?.sync();
  ...
  openEditProfileModal(userId) {
    ...
    container.innerHTML = `
      <div class="Administrator-inline-form-box" style="margin-top:20px; border-color:var(--secondary-color);">
        ...
        <form onsubmit="AdministratorModule.handleSaveProfileOverride(event, '${u.id}')" style="display:flex; flex-direction:column; gap:12px;">
          <div style="display:grid; grid-template-columns: 1fr 1fr; gap:12px;">
            <div class="form-group" style="margin-bottom:0;">
  ...
  async handleSaveProfileOverride(e, userId) {
    e.preventDefault();
    const u = window.cbsDB.getUserById(userId);
    if (u) {
  ...
  renderChannels(area) {
    ...
    channels.forEach(ch => {
      ...
      const deleteBtn = !isDefault ?
        `<button onclick="AdministratorModule.handleDeleteChannel('${ch.id}')" class="badge badge-red" style="border:none; cursor:pointer;">Delete</button>` :
      ...
      rowsHtml += `
        <tr>
          ...
          <td>
            <div class="card-actions" style="gap:8px;">
              <button onclick="AdministratorModule.handleRenameChannel('${ch.id}')" class="badge badge-purple" style="border:none; cursor:pointer;">Rename</button>
              <button onclick="AdministratorModule.handleManageChannelMembers('${ch.id}')" class="badge badge-blue" style="border:none; cursor:pointer;">Members</button>
              ${deleteBtn}
            </div>
          </td>
        </tr>
    ...
    area.innerHTML = `
      <div style="display:flex; flex-direction:column; gap:16px;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          ...
          <button onclick="AdministratorModule.openCreateChannelInline()" class="btn-primary" style="padding:6px 12px; font-size:11.5px;"><i class="fa-solid fa-plus"></i> Add Channel</button>
        </div>
  ...
  openCreateChannelInline() {
    ...
    container.innerHTML = `
      <form onsubmit="AdministratorModule.handleSaveChannelInline(event)" style="background:var(--bg-main); border:1px solid var(--border-color); padding:12px; border-radius:var(--radius-md); margin-bottom:12px; display:flex; flex-direction:column; gap:8px;">
        <div style="display:flex; gap:8px;">
  ...
  async handleSaveChannelInline(e) {
    e.preventDefault();
    ...
    await window.cbsDB.saveChannel({ name, description: desc, members: [] });
    document.getElementById('inlineChannelFormBox').innerHTML = '';
    window.cbsApp?.sync();
  },

  async handleRenameChannel(id) {
    ...
    if (newName) {
      ...
      await window.cbsDB.saveChannel(ch);
      window.cbsApp?.sync();
    }
  },
  ...
  async handleDeleteChannel(id) {
    if (confirm('Delete this chat channel and erase all its messages? This cannot be restored.')) {
      await window.cbsDB.deleteChannel(id);
      window.cbsApp?.sync();
    }
  },
  ...
  handleManageChannelMembers(channelId) {
    ...
    users.forEach(u => {
      ...
      checklistHtml += `
        <div style="display:flex; align-items:center; gap:8px;">
          <input type="checkbox" id="chk-ch-mem-${u.id}" value="${u.id}" ${isChecked} onchange="AdministratorModule.handleToggleChannelMember('${ch.id}', '${u.id}')" style="cursor:pointer;">
    ...
    container.innerHTML = `
      <div class="Administrator-inline-form-box" style="margin-top:16px; border-color:var(--primary-color);">
  ...
  async handleToggleChannelMember(channelId, userId) {
    const channels = window.cbsDB.getChannels();
    const ch = channels.find(c => c.id === channelId);
    if (ch) {
      ...
      itemsHtml += `
        <div style="display:flex; justify-content:space-between; align-items:center; background:var(--bg-main); border:1px solid var(--border-color); padding:12px; border-radius:var(--radius-md); opacity: ${n.read ? '0.7' : '1'};">
          ...
          <div style="display:flex; gap:8px; align-items:center;">
            ...
            <button onclick="AdministratorModule.handleToggleNotifRead('${n.id}', ${!n.read})" class="badge badge-blue" style="border:none; cursor:pointer;">
              ${n.read ? 'Unread' : 'Read'}
            </button>
            <button onclick="AdministratorModule.handleDeleteNotif('${n.id}')" class="action-icon-btn delete" style="padding:4px;"><i class="fa-regular fa-trash-can"></i></button>
          </div>
        </div>
      `;
    });
    ...
    area.innerHTML = `
      <div style="display:flex; flex-direction:column; gap:16px;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          ...
          <div style="display:flex; gap:8px;">
            <button onclick="AdministratorModule.openCreateNotificationInline()" class="btn-primary" style="padding:6px 12px; font-size:11.5px;"><i class="fa-solid fa-bullhorn"></i> Push Alert</button>
            <button onclick="AdministratorModule.handleMarkAllNotifsRead()" class="btn-secondary" style="padding:6px 12px; font-size:11.5px;">Mark All Read</button>
          </div>
        </div>
        ...
        <div style="display:flex; gap:10px; flex-wrap:wrap; align-items:center; background:var(--bg-main); padding:12px; border-radius:var(--radius-md); border:1px solid var(--border-color); margin-bottom:20px;">
          <div class="topbar-search" style="display:block; width:180px;">
            <i class="fa-solid fa-magnifying-glass"></i>
            <input type="text" id="admNotifSearch" placeholder="Filter alerts..." value="${searchVal}" oninput="AdministratorModule.switchAdministratorTab('notifications')" style="padding: 6px 12px 6px 30px; font-size:11.5px;">
          </div>
          
          <select id="admNotifCatFilter" onchange="AdministratorModule.switchAdministratorTab('notifications')" style="padding:6px; font-size:11.5px; border:1px solid var(--border-color); border-radius:var(--radius-sm);">
  ...
  openCreateNotificationInline() {
    ...
    container.innerHTML = `
      <div class="Administrator-inline-form-box">
        ...
        <form onsubmit="AdministratorModule.handleCreateNotificationSubmit(event)" style="display:flex; flex-direction:column; gap:10px;">
          <div style="display:grid; grid-template-columns: 1fr 1fr; gap:12px;">
            <div class="form-group" style="margin-bottom:0;">
  ...
  async handleCreateNotificationSubmit(e) {
    ...
    await window.cbsDB.addNotification(cat, title, msg, pri);
    document.getElementById('inlineNotifFormBox').innerHTML = '';
    window.cbsApp?.sync();
  },

  async handleToggleNotifRead(id, readVal) {
    const list = window.cbsDB.getNotifications();
    const idx = list.findIndex(n => n.id === id);
    if (idx !== -1) {
      list[idx].read = readVal;
      // Persist change via Supabase-backed API
      await window.cbsDB.saveNotification(list[idx]);
      window.cbsApp?.sync();
    }
  },
    ...
    area.innerHTML = `
      <div style="display:flex; flex-direction:column; gap:16px;">
        ...
        <div style="display:flex; gap:10px; flex-wrap:wrap; align-items:center; background:var(--bg-main); padding:12px; border-radius:var(--radius-md); border:1px solid var(--border-color); margin-bottom:20px;">
          <div class="topbar-search" style="display:block; width:180px;">
            <i class="fa-solid fa-magnifying-glass"></i>
            <input type="text" id="admLgSearch" placeholder="Filter message..." value="${searchVal}" oninput="AdministratorModule.switchAdministratorTab('activity')" style="padding: 6px 12px 6px 30px; font-size:11.5px;">
          </div>

          <select id="admLgUserFilter" onchange="AdministratorModule.switchAdministratorTab('activity')" style="padding:6px; font-size:11.5px; border:1px solid var(--border-color); border-radius:var(--radius-sm);">
            ${usersOpts}
          </select>

          <select id="admLgCatFilter" onchange="AdministratorModule.switchAdministratorTab('activity')" style="padding:6px; font-size:11.5px; border:1px solid var(--border-color); border-radius:var(--radius-sm);">
            <option value="All" ${catVal === 'All' ? 'selected' : ''}>All Categories</option>
            <option value="Security" ${catVal === 'Security' ? 'selected' : ''}>Security</option>
          ...
          <input type="date" id="admLgDateFilter" value="${dateVal}" onchange="AdministratorModule.switchAdministratorTab('activity')" style="padding:5px; font-size:11.5px; border:1px solid var(--border-color); border-radius:var(--radius-sm);">
        </div>
      ...
      <div style="display:flex; flex-direction:column; gap:20px;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          ...
          <div style="display:flex; gap:8px;">
            ...
            <button onclick="AdministratorModule.handleExportCSV(document.getElementById('admReportSelect').value)" class="btn-primary" style="padding:6px 12px; font-size:11.5px; background:var(--color-success);"><i class="fa-solid fa-file-csv"></i> Export CSV</button>
            <button onclick="AdministratorModule.handleExportPDF(document.getElementById('admReportSelect').value)" class="btn-primary" style="padding:6px 12px; font-size:11.5px; background:var(--color-success);"><i class="fa-solid fa-print"></i> Export PDF</button>
          </div>
        </div>

        <!-- 5 Charts Grid -->
        <div class="Administrator-grid-charts">
          <div class="Administrator-chart-box">
            <h4 style="font-size:12px; font-weight:700; margin-bottom:12px;">Project Completion (Completed vs Active)</h4>
            <div style="flex:1; display:flex; align-items:center; justify-content:center; min-height:0;">
              <canvas id="AdministratorChartProjectCompletion"></canvas>
            </div>
          </div>
          <div class="Administrator-chart-box">
            <h4 style="font-size:12px; font-weight:700; margin-bottom:12px;">Team Productivity (Completed tasks per member)</h4>
            <div style="flex:1; display:flex; align-items:center; justify-content:center; min-height:0;">
              <canvas id="AdministratorChartTeamProductivity"></canvas>
            </div>
          </div>
          <div class="Administrator-chart-box">
            <h4 style="font-size:12px; font-weight:700; margin-bottom:12px;">User Activity (Audit logs frequency per user)</h4>
            <div style="flex:1; display:flex; align-items:center; justify-content:center; min-height:0;">
              <canvas id="AdministratorChartUserActivity"></canvas>
            </div>
          </div>
          <div class="Administrator-chart-box">
            <h4 style="font-size:12px; font-weight:700; margin-bottom:12px;">Task Distribution (Statuses columns)</h4>
            <div style="flex:1; display:flex; align-items:center; justify-content:center; min-height:0;">
              <canvas id="AdministratorChartTaskDistribution"></canvas>
            </div>
          </div>
          <div class="Administrator-chart-box" style="grid-column: span 2;">
            ...
            <div style="flex:1; display:flex; align-items:center; justify-content:center; min-height:0; height:180px;">
              <canvas id="AdministratorChartWorkspaceGrowth"></canvas>
            </div>
          </div>
        </div>
      </div>
  ...
  drawAnalyticsCharts() {
    ...
    const ctx1 = document.getElementById('AdministratorChartProjectCompletion');
    if (ctx1) {
    ...
    const ctx2 = document.getElementById('AdministratorChartTeamProductivity');
    if (ctx2) {
    ...
    const ctx3 = document.getElementById('AdministratorChartUserActivity');
    if (ctx3) {
    ...
    const ctx4 = document.getElementById('AdministratorChartTaskDistribution');
    if (ctx4) {
    ...
    const ctx5 = document.getElementById('AdministratorChartWorkspaceGrowth');
    if (ctx5) {
  ...
  handleExportCSV(reportType) {
    ...
    if (reportType === 'Users') {
      ...
      csvContent += "Chart,Metric,Value\n";
      csvContent += `"Project Completion","Completed Projects",${completed}\n`;
      csvContent += `"Project Completion","Active/Other Projects",${projects.length - completed}\n`;
      csvContent += `"Project Completion","Total Projects",${projects.length}\n`;
      users.forEach(u => {
        const done = tasks.filter(t => t.assigneeId === u.id && t.status === 'Completed').length;
        csvContent += `"Team Productivity","${u.name} - Completed Tasks",${done}\n`;
      });
      users.forEach(u => {
        const freq = logs.filter(l => l.userId === u.id).length;
        csvContent += `"User Activity","${u.name} - Activity Logs",${freq}\n`;
      });
      const statuses = ['Completed', 'In Progress', 'In Review', 'To Do', 'Backlog'];
      statuses.forEach(s => {
        csvContent += `"Task Distribution","${s}",${tasks.filter(t => t.status === s).length}\n`;
      });
      csvContent += `"Workspace Growth","Total Users",${users.length}\n`;
    }
  ...
  handleExportPDF(reportType) {
    ...
    let reportHtml = `
      ...
      <head>
        ...
        <style>
          ...
          h3 { color: #1e293b; margin-top: 32px; border-bottom: 1px solid #e2e8f0; padding-bottom: 6px; }
          table { width: 100%; border-collapse: collapse; margin-top: 16px; }
          th, td { border: 1px solid #ddd; padding: 8px 12px; text-align: left; font-size: 13px; }
          th { background: #f8fafc; font-weight: bold; }
          .charts-section { margin-top: 36px; page-break-before: always; }
          .chart-block { margin-bottom: 28px; text-align: center; }
          .chart-block h4 { font-size: 13px; font-weight: 700; margin-bottom: 10px; color: #334155; }
          .chart-block img { max-width: 100%; max-height: 280px; border: 1px solid #e2e8f0; border-radius: 8px; padding: 8px; background: #fff; }
          @media print { .charts-section { page-break-before: always; } }
        </style>
      </head>
      <body>
    ...
    if (reportType === 'Users') {
      ...
      // Analytics Charts — no table, only charts below
      reportHtml += `</thead><tbody>`;
    }

    reportHtml += `</tbody></table>`;

    // Add Live Growth Analytics charts for Analytics report type
    if (reportType === 'Analytics') {
      const chartIds = [
        { id: 'AdministratorChartProjectCompletion', title: 'Project Completion (Completed vs Active)' },
        { id: 'AdministratorChartTeamProductivity', title: 'Team Productivity (Completed tasks per member)' },
        { id: 'AdministratorChartUserActivity', title: 'User Activity (Audit logs frequency per user)' },
        { id: 'AdministratorChartTaskDistribution', title: 'Task Distribution (Status columns)' },
        { id: 'AdministratorChartWorkspaceGrowth', title: 'Workspace Growth (User Enrollment Timeline)' }
      ];
      const chartImages = [];
      chartIds.forEach(c => {
        const canvas = document.getElementById(c.id);
        if (canvas) {
          try { chartImages.push({ title: c.title, src: canvas.toDataURL('image/png') }); } catch(e) {}
        }
      });
      ...
      if (chartImages.length > 0) {
        reportHtml += `
          <div class="charts-section">
            <h3>📊 Live Growth Analytics & Reports</h3>
            <p style="color:#64748b; font-size:12px;">Visual analytics snapshot captured at time of export.</p>
        `;
  ...
  renderBroadcasts(area) {
    area.innerHTML = `
      <div style="max-width:540px; margin:0 auto; background:var(--bg-main); border:1px solid var(--border-color); padding:20px; border-radius:var(--radius-lg);">
        ...
        <form onsubmit="AdministratorModule.handleBroadcastAnnouncementSubmit(event)" style="display:flex; flex-direction:column; gap:12px;">
  ...
  async handleBroadcastAnnouncementSubmit(e) {
    e.preventDefault();
    ...
    await window.cbsDB.addNotification('System', `[Broadcast] ${title}`, `${msg} (Scope: ${audience})`, pri);

    // 2. Add activity log
    await window.cbsDB.addActivityLog(me.id, `Broadcasted announcement: "${title}" to ${audience}`, 'System');

    // 3. Post to general discussions chat channel
    const channels = window.cbsDB.getChannels();
    const generalChannel = channels.find(c => c.name === 'general') || channels[0];
    if (generalChannel) {
      await window.cbsDB.addChatMessage(generalChannel.id, me.id, `🚨 **[WORKSPACE BROADCAST - ${pri}]**: **${title}**\n${msg}`);
    }
  ...
  renderSecurity(area) {
    ...
    sessions.forEach(s => {
      ...
      let actionBtn = '';
      if (isSuccess) {
        actionBtn = `<button onclick="AdministratorModule.handleForceLogout('${s.id}')" class="badge badge-red" style="border:none; cursor:pointer;">Force Logout</button>`;
      } else {
        actionBtn = `<button onclick="AdministratorModule.handleDeleteSessionRecord('${s.id}')" class="action-icon-btn delete" style="padding:2px;"><i class="fa-regular fa-trash-can"></i></button>`;
      }
      ...
      rowsHtml += `
        <tr>
          ...
          <td>
            <div class="card-actions" style="gap:8px;">
              ${actionBtn}
              ${u.status !== 'suspended' && u.id !== window.cbsAuth.getCurrentUser()?.id ? `<button onclick="AdministratorModule.handleSuspendUserFromSecurity('${u.id}')" class="badge badge-orange" style="border:none; cursor:pointer;">Suspend Account</button>` : ''}
            </div>
          </td>
        </tr>
      `;
  ...
  async handleForceLogout(sessionId) {
    if (confirm('Verify: Terminate this session and force user logout?')) {
      const sessions = window.cbsDB.getSessions();
      const s = sessions.find(sess => sess.id === sessionId);
      if (s) {
        // Update session status through DB layer
        await window.cbsDB.updateSessionStatus(s.id, 'Terminated');

        // Log action
        await window.cbsDB.addActivityLog(window.cbsAuth.getCurrentUser()?.id, `Administratively terminated session for User ID: ${s.userId}`, 'Security');
        window.cbsApp?.sync();
  ...
  async handleDeleteSessionRecord(sessionId) {
    await window.cbsDB.deleteSession(sessionId);
    window.cbsApp?.sync();
  },
  ...
  renderSettings(area) {
    ...
    area.innerHTML = `
      <div style="max-width:540px; margin:0 auto; display:flex; flex-direction:column; gap:20px;">
        ...
        <form onsubmit="AdministratorModule.handleSaveSettingsSubmit(event)" style="display:flex; flex-direction:column; gap:16px;">
  ...
  async handleSaveSettingsSubmit(e) {
    e.preventDefault();
    const settings = window.cbsDB.getSettings();
    ...
    await window.cbsDB.saveSettings(settings);
    ...
    await window.cbsDB.addActivityLog(window.cbsAuth.getCurrentUser()?.id, `Updated workspace settings preferences.`, 'System');
    window.cbsApp?.sync();
...
window.AdministratorModule = AdministratorModule;
