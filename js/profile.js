...
const profileModule = {
  ...
  render() {
    ...
    // Lock/Unlock profile fields depending on whether active user is an Administrator
    ...
    if (profileForm) {
      ...
      // Save button is always visible so everyone can save their details
      const saveBtnRow = profileForm.querySelector('.form-actions-row');
      if (saveBtnRow) {
    ...
    if (avatarImg) {
      ...
      if (!avatarImg.dataset.boundClick) {
  ...
  async handleSaveSubmit(e) {
    ...
    // Programmatic override check: non-administrators cannot change their department
    if (!window.cbsAuth.isAdministrator()) {
      updated.department = user.department || 'General';
    }

    try {
      await window.cbsDB.saveUser({ ...user, ...updated });
      window.cbsDB.addActivityLog(user.id, `${updated.name} updated their personal profile fields.`, 'User Management').catch(err => console.warn('Activity log error:', err));

      // Sync headers
      const sidebarName = document.getElementById('sidebarUserName');
      if (sidebarName) sidebarName.textContent = updated.name;
      const sidebarRole = document.getElementById('sidebarUserRole');
      if (sidebarRole) sidebarRole.textContent = updated.title || user.role;
  ...
  async handleSaveCroppedAvatarSubmit() {
    if (!this.tempBase64Image) return;

    const user = window.cbsAuth.getCurrentUser();
    user.avatar = this.tempBase64Image;
    await window.cbsDB.saveUser(user);
    window.cbsDB.addActivityLog(user.id, `${user.name} changed their display profile picture avatar.`, 'User Management').catch(err => console.warn('Activity log error:', err));
