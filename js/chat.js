...
const chatModule = {
  ...
  async handleSendChatMessage(e) {
    ...
    await window.cbsDB.addChatMessage(this.activeChannelId, user.id, text);
    input.value = '';
  ...
  async handleMessageReaction(msgId, emoji) {
    ...
    await window.cbsDB.saveMessage(msg);
    window.cbsApp?.sync();
  },
  ...
  async saveInlineEditSubmit(msgId) {
  ...
  async handleDeleteMessage(id) {
    if (confirm('Delete this message permanently?')) {
      await window.cbsDB.deleteMessage(id);
      window.cbsApp?.sync();
    }
  },
  ...
  async openCreateChannelModal() {
    ...
    await window.cbsDB.saveChannel({
      name: cleanName,
      description: desc || ''
    });
  ...
  async handleRenameChannel(e, chId) {
    e.stopPropagation();
    e.preventDefault();
