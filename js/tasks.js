...
const tasksModule = {
  render() {
    ...
    filtered.forEach(t => {
      ...
      // Security checking: Admins and Founders can modify all tasks. Members cannot edit or drag tasks.
  ...
  handleSaveSubmit(e) {
    ...
    window.cbsDB.saveTask(taskData);
  ...
  handleUploadAttachment(input) {
    ...
    if (t) {
      ...
      window.cbsDB.addNotification('Tasks', 'File Attached', `File "${name}" uploaded to task "${t.title}".`, 'Low');
